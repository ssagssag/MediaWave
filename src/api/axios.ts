...
export const getTrendingMovie = async (timeWindow: 'day' | 'week') => {
  try{
    const {data: {results}} = await axiosInstance.get(`/trending/movie/${timeWindow}`)
    return results;
  } catch (error){
...
export const getTvAiringToday = async () => {
  try{
    const {data: {results}} = await axiosInstance.get("/tv/airing_today");
    return results;
  } catch (error){
...
export const getTvOnTheAir = async () => {
  try{
    const {data: {results}} = await axiosInstance.get("/tv/on_the_air",);
    return results;
  } catch (error){
